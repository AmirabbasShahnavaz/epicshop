...
function config() {
    ...
    $config.endtime_message = "<a class='span-primary' style='cursor: pointer'>درخواست کد جدید</a>";
    ...
    var interval = setInterval(function () {
        ...
        if ($newDistance == $config.loadingBars_width) {
            ...
            setTimeout(function () {
                ...
                $link.attr('onclick', `resendCode('${url}')`);
            }, 100);
...
function getResendData() {
    return {
        count: parseInt(localStorage.getItem('resendCount') || '1'),
    };
}
    ...
    if (count <= 3) {
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                } else if (data.status === 'send_code_failed') {
                    Swal.fire({
                        title: 'خطا',
                        text: data.message,
                        icon: "error",
        ...
        localStorage.clear();
        window.location.href = `/mobile_activation/1fdb71114be07434c0cc`;
    }
}